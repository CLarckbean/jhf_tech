<!DOCTYPE html>
<html lang="pt-br">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Loja Virtual</title>
    <link rel="stylesheet" href="assets/css/style.css">
    <link rel="stylesheet" href="assets/css/banner.css">
    <link rel="stylesheet" href="assets/css/categorias.css">
    <!-- <link rel="stylesheet" href="assets/css/productos.css"> -->
    <link rel="stylesheet" href="assets/css/ofertas.css">
    <link rel="stylesheet" href="assets/css/depoimentos.css">
    <link rel="stylesheet" href="assets/css/marcas.css">
    <link rel="stylesheet" href="assets/css/rodape.css">
    <link rel="stylesheet" href="assets/css/media.css">
</head>
<body>
    <section class="banner">
        <div class="container">
            <!-- START OF NAVBAR -->
            <div class="navbar">
                <div class="logo">
                    <a href="index.html"><img src="assets/img/logo.png" alt="" width="125px"></a> <!--Alterar o alt-->
                </div>

                <nav>
                    <ul id="menuItens">
                        <li><a href="index.php" title="">Início</a></li>
                        <li><a href="productos.php" title="">Productos</a></li>
                        <li><a href="sobre.php" title="">Sobre</a></li>
                        <li><a href="contactos.php" title="">Contactos</a></li>
                        <li><a href="minha-conta.php" title="">Minha Conta</a></li>
                    </ul>
                </nav>
                <!-- fim navegação -->